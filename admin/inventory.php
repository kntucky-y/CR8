...
$sql = "
    SELECT 
        p.id, p.product_name, p.base_variant_name, p.image, p.quantity,
        a.artist_name
    FROM products p
...
<aside class="sticky top-0 w-64 bg-white border-r flex-shrink-0 hidden md:flex flex-col justify-between h-screen z-40">
        <div>
    ...
    <main class="flex-1 flex flex-col min-h-screen">
        ...
        <section class="p-4 md:p-8">
            <form method="GET" class="bg-white p-4 rounded-xl shadow mb-8 flex flex-col md:flex-row items-center gap-4">
                <input type="text" name="search" value="<?= htmlspecialchars($search_term) ?>" placeholder="Search by product name..." class="border-gray-300 rounded-md shadow-sm w-full md:w-1/3">
                            ...
                            <?php if ($products_result && $products_result->num_rows > 0): ?>
                                <?php while($product = $products_result->fetch_assoc()): ?>
                                    <tr class="hover:bg-gray-50" id="product-row-<?= $product['id'] ?>">
                                        ...
                                        <td class="p-4 text-center font-semibold <?= $product['quantity'] <= 0 ? 'text-red-500' : ($product['quantity'] <= 10 ? 'text-yellow-600' : 'text-green-600') ?>">
                                            <?= $product['quantity'] ?>
                                        </td>
                                        <td class="p-4 text-center">
                                            <button data-id="<?= $product['id'] ?>" data-name="<?= htmlspecialchars($product['product_name']) ?>" class="delete-product-btn text-red-500 hover:text-red-700 font-semibold">Delete</button>
                                        </td>
                                    </tr>
                                <?php endwhile; ?>
                            <?php else: ?>
                                <tr><td colspan="4" class="text-center p-8 text-gray-500">No products found for the selected filters.</td></tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
                </div>
    ...
    if (inventoryTable) {
        inventoryTable.addEventListener('click', async (e) => {
            if (e.target.classList.contains('delete-product-btn')) {
                ...
                if (confirm(`Are you sure you want to permanently delete "${productName}" and all of its variants? This action cannot be undone.`)) {
                    try {
