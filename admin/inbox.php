...
// Set current page for highlighting
$current_page = 'inbox';
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const msgList = document.querySelector('.overflow-y-auto.flex-1');
    if (msgList && localStorage.getItem('msgScroll')) {
        msgList.scrollTop = parseInt(localStorage.getItem('msgScroll'), 10);
    }
    // Save scroll position on scroll
    msgList?.addEventListener('scroll', function() {
        localStorage.setItem('msgScroll', msgList.scrollTop);
    });
});
</script>
...
<aside class="w-64 bg-white border-r flex-shrink-0 flex flex-col justify-between sticky top-0 h-screen">
    ...
    <main class="flex-1 flex flex-col h-screen">
        ...
        <section class="flex-1 flex bg-white overflow-hidden">
            <div class="w-96 border-r flex flex-col h-full">
                <div class="p-4 border-b sticky top-0 bg-white z-20">
                    <h2 class="font-bold text-lg">
                        Customer Messages 
                        <span class="<?= $unread_messages_count > 0 ? 'bg-blue-100 text-blue-700' : 'bg-gray-200 text-gray-400' ?> text-xs px-2 py-1 rounded-full">
